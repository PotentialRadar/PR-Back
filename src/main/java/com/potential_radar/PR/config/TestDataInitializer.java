...
//        }
//
//        // 좋아요 데이터 초기화 (프로젝트 생성 후 1회)
//        if (likeRepository.count() == 0) {
//            initializeLikes();
//        } else {
//            log.info("Skip likes (already present)");
//        }
//
...
//
//    /**
//     * 프로젝트에 대한 사용자 좋아요 데이터를 초기화합니다.
//     * 현실적인 사용자 행동 패턴을 반영하여 3가지 유형의 좋아요를 생성합니다:
//     *
//     * 1. 기술스택 유사성 기반 (70-80%): 사용자가 보유한 기술스택과 겹치는 프로젝트
//     *    - 가장 현실적인 패턴으로 추천 시스템 정확도 향상에 기여
//     *
//     * 2. 같은 분야 다른 기술 (15-20%): 같은 tech part이지만 다른 기술스택 사용
//     *    - 새로운 기술 학습 욕구나 협업 의지를 반영
//     *
//     * 3. 완전히 다른 분야 (5-10%): 전혀 다른 tech part의 프로젝트
//     *    - 호기심이나 새로운 도전 의욕을 반영
//     */
//    private void initializeLikes() {
//        log.info("Creating user likes with realistic behavior patterns ...");
//
//        List<User> users = userRepository.findAll();
//        List<ProjectRecruitment> projects = projectRecruitmentRepository.findAll();
//        Random random = new Random();
...
//            log.error("사용자나 프로젝트 데이터가 존재하지 않아 좋아요를 생성할 수 없습니다.");
//            return;
//        }
//
//        int totalLikes = 0;
//
//        // 100명 중 50명만 좋아요를 누르도록 제한 (현실적인 사용자 활동 반영)
//        Collections.shuffle(users, random);
//        List<User> activeUsers = users.subList(0, Math.min(50, users.size()));
//
//        for (User user : activeUsers) {
//            // 각 사용자가 좋아요를 누를 프로젝트 개수 (3~15개, 가중 분포)
//            int likeCount = generateWeightedLikeCount(random);
//
//            // 사용자의 기술스택 정보 가져오기
//            List<UserTechStack> userTechStacks = userTechStackRepository.findByUserUserId(user.getUserId());
//            Set<String> userTechNames = new HashSet<>();
//            for (UserTechStack uts : userTechStacks) {
//                userTechNames.add(uts.getStack().getName());
//            }
//
//            // 사용자의 tech part 정보
//            String userTechPart = user.getTechPart() != null ? user.getTechPart().getName() : "";
//
//            Set<Long> likedProjectIds = new HashSet<>();
//
//            for (int i = 0; i < likeCount && likedProjectIds.size() < projects.size(); i++) {
//                ProjectRecruitment targetProject = selectProjectByStrategy(
//                    user, projects, userTechNames, userTechPart, likedProjectIds, random
//                );
//
...
//                            .user(user)
//                            .targetId(targetProject.getProjectId())
//                            .targetType(TargetType.PROJECT)
//                            .build());
//
//                    likedProjectIds.add(targetProject.getProjectId());
//                    totalLikes++;
//                }
//            }
//        }
//
//        log.info("User likes created: {} (average {:.1f} likes per active user, {} out of {} users are active)",
//                totalLikes, (double) totalLikes / activeUsers.size(), activeUsers.size(), users.size());
//    }
//
//    /**
//     * 사용자별 좋아요 개수를 현실적인 분포로 생성합니다.
//     * - 50%: 3-7개 (일반적인 사용자)
//     * - 30%: 8-12개 (활발한 사용자)
//     * - 20%: 13-15개 (매우 활발한 사용자)
//     */
//    private int generateWeightedLikeCount(Random random) {
//        int dice = random.nextInt(100);
//        if (dice < 50) {
//            return random.nextInt(5) + 3;  // 3-7개
//        } else if (dice < 80) {
//            return random.nextInt(5) + 8;  // 8-12개
//        } else {
//            return random.nextInt(3) + 13; // 13-15개
//        }
//    }
//
//    /**
//     * 사용자 행동 패턴에 따라 좋아요할 프로젝트를 선택합니다.
//     *
//     * @param user 사용자
//     * @param projects 전체 프로젝트 목록
//     * @param userTechNames 사용자 기술스택 이름 집합
//     * @param userTechPart 사용자 tech part
//     * @param likedProjectIds 이미 좋아요한 프로젝트 ID 집합
//     * @param random 랜덤 객체
//     * @return 선택된 프로젝트 (null 가능)
//     */
//    private ProjectRecruitment selectProjectByStrategy(
//            User user, List<ProjectRecruitment> projects,
//            Set<String> userTechNames, String userTechPart,
//            Set<Long> likedProjectIds, Random random) {
//
//        int strategy = random.nextInt(100);
//
//        if (strategy < 75) {
//            // 전략 1: 기술스택 유사성 기반 (75%)
//            return selectSimilarTechProject(projects, userTechNames, likedProjectIds, random);
//        } else if (strategy < 90) {
//            // 전략 2: 같은 분야 다른 기술 (15%)
//            return selectSamePartDifferentTechProject(projects, userTechPart, userTechNames, likedProjectIds, random);
//        } else {
//            // 전략 3: 완전히 다른 분야 (10%)
//            return selectDifferentPartProject(projects, userTechPart, likedProjectIds, random);
//        }
//    }
//
//    /**
//     * 사용자 기술스택과 겹치는 기술을 사용하는 프로젝트를 선택합니다.
//     */
//    private ProjectRecruitment selectSimilarTechProject(
//            List<ProjectRecruitment> projects, Set<String> userTechNames,
//            Set<Long> likedProjectIds, Random random) {
//
//        List<ProjectRecruitment> candidates = new ArrayList<>();
//
//        for (ProjectRecruitment project : projects) {
//            if (likedProjectIds.contains(project.getProjectId())) {
//                continue;
//            }
//
//            // 프로젝트의 기술스택 조회
//            List<ProjectTechStack> projectTechStacks =
//                projectTechStackRepository.findByProject_ProjectId(project.getProjectId());
//
//            // 겹치는 기술스택이 있는지 확인
//            boolean hasCommonTech = projectTechStacks.stream()
//                .anyMatch(pts -> userTechNames.contains(pts.getTechStack().getName()));
//
//            if (hasCommonTech) {
...
//
//    /**
//     * 같은 tech part이지만 다른 기술스택을 사용하는 프로젝트를 선택합니다.
//     */
//    private ProjectRecruitment selectSamePartDifferentTechProject(
//            List<ProjectRecruitment> projects, String userTechPart,
//            Set<String> userTechNames, Set<Long> likedProjectIds, Random random) {
//
//        if (userTechPart.isEmpty()) {
//            return null;
//        }
//
//        List<ProjectRecruitment> candidates = new ArrayList<>();
...
//
//            // 프로젝트의 tech part 조회
//            List<ProjectTechPart> projectTechParts =
//                projectTechPartRepository.findByProject_ProjectId(project.getProjectId());
//
//            boolean hasSamePart = projectTechParts.stream()
//                .anyMatch(ptp -> userTechPart.equals(ptp.getTechPart().getName()));
//
//            if (hasSamePart) {
//                // 기술스택이 다른지 확인 (새로운 기술 학습 욕구)
//                List<ProjectTechStack> projectTechStacks =
//                    projectTechStackRepository.findByProject_ProjectId(project.getProjectId());
//
//                boolean hasUnknownTech = projectTechStacks.stream()
//                    .anyMatch(pts -> !userTechNames.contains(pts.getTechStack().getName()));
//
//                if (hasUnknownTech) {
//                    candidates.add(project);
...
//
//    /**
//     * 완전히 다른 tech part의 프로젝트를 선택합니다 (호기심/도전정신).
//     */
//    private ProjectRecruitment selectDifferentPartProject(
//            List<ProjectRecruitment> projects, String userTechPart,
//            Set<Long> likedProjectIds, Random random) {
//
//        if (userTechPart.isEmpty()) {
//            return selectRandomProject(projects, likedProjectIds, random);
//        }
//
//        List<ProjectRecruitment> candidates = new ArrayList<>();
//
...
//                projectTechPartRepository.findByProject_ProjectId(project.getProjectId());
//
//            boolean hasDifferentPart = projectTechParts.stream()
//                .anyMatch(ptp -> !userTechPart.equals(ptp.getTechPart().getName()));
//
//            if (hasDifferentPart) {
//                candidates.add(project);
...
//            selectRandomProject(projects, likedProjectIds, random) :
//            candidates.get(random.nextInt(candidates.size()));
//    }
//
//    /**
//     * 랜덤하게 프로젝트를 선택합니다 (마지막 수단).
//     */
//    private ProjectRecruitment selectRandomProject(
//            List<ProjectRecruitment> projects, Set<Long> likedProjectIds, Random random) {
//
//        List<ProjectRecruitment> candidates = projects.stream()
//            .filter(p -> !likedProjectIds.contains(p.getProjectId()))
//            .toList();
//
//        return candidates.isEmpty() ? null : candidates.get(random.nextInt(candidates.size()));
//    }
