...
public interface UserProfileRepository extends JpaRepository<UserProfile, Long> {
    ...
    // 공개된 모든 포트폴리오 조회 (페이징)
    @Query("SELECT up FROM UserProfile up WHERE up.isPortfolioOpen = true")
    Page<UserProfile> findAllPublicPortfolios(Pageable pageable);
    
    // 기술 파트별 공개 포트폴리오 조회
    @Query("SELECT up FROM UserProfile up WHERE up.isPortfolioOpen = true AND up.techPart.name = :techPartName")
    Page<UserProfile> findPublicPortfoliosByTechPart(@Param("techPartName") String techPartName, Pageable pageable);
    
    // 경력 범위별 공개 포트폴리오 조회
    @Query("SELECT up FROM UserProfile up WHERE up.isPortfolioOpen = true AND up.experienceRange = :experienceRange")
